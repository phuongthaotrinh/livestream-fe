...
export function ShellAction({
    ...
    return isLink && href ? (
        <Link key={id} aria-label="back" href={href} className={clsx("flex w-full", {  "justify-end":!flex })}>
            <div {...buttonProps}>{renderContent}</div>
        </Link>
    ) : (
        <div key={id} onClick={actionVoid} className={clsx("cursor-pointer", buttonProps.className)} {...props}>
            {renderContent}
        </div>
    );
}
